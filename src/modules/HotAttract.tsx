...
export default function HotAttract({ hotAttrData }: HotAttrDataProps) {
  return (
    <div>
      <div className="container pb-[100px] lg:pb-[160px]">
        ...
        <ul className="flex flex-col md:flex-row space-y-10 md:space-y-0 md:space-x-5 lg:space-x-6 items-center justify-between cursor-pointer">
          {hotAttrData.map((item, index) => {
            return (
              <div
                key={index}
                className="w-full md:w-1/2 lg:min-w-[360px]  md:px-0 md:[&:nth-child(3)]:hidden lg:[&:nth-child(3)]:block"
              >
                <AttrCard
